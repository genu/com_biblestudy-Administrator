...
<tr>
    ...
    <th>
        <?php echo JHtml::_('grid.sort', '*RESOURCE NAME*', 'mediafile.filename', $listDirn, $listOrder); ?>
    </th>
    ...
    <th width="5%" class="nowrap center hidden-phone hidden-tablet">
        <?php echo JHtml::_('grid.sort', '*Statistics*', 'mediafile.plays', $listDirn, $listOrder); ?>
    </th>
</tr>
</thead>
<tbody>
            ...
            <div class="pull-left">
                <?php if ($canEdit || $canEditOwn) : ?>
                    <a href="<?php echo JRoute::_('index.php?option=com_biblestudy&task=mediafile.edit&id=' . (int)$item->id); ?>">
                        						<?php echo($this->escape($item->id) ? $this->escape($item->id) : 'ID: ' . $this->escape($item->id)); ?>
                    </a>
                <?php else : ?>
                    					<?php echo($this->escape($item->id) ? $this->escape($item->id) : 'ID: ' . $this->escape($item->id)); ?>
                <?php endif; ?>
